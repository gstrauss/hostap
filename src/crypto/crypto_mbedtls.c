...
struct crypto_ecdh * crypto_ecdh_init(int group)
{
	...
	if (mbedtls_ecdh_setup(ecdh, grp_id) == 0)
...
#define MBEDTLS_PK_ECP_PUB_DER_MAX_BYTES    ( 30 + 2 * MBEDTLS_ECP_MAX_BYTES )
	 ...
	 *        using the buffer */
	unsigned char *der = buf+sizeof(buf)-len;
	...
	if (mbedtls_ecp_get_type(grp) == MBEDTLS_ECP_TYPE_SHORT_WEIERSTRASS) {
		...
		size_t n = 2+1+plen*2;
		len -= (int)plen;
		der[1] -= (unsigned char)plen;
		buf[sizeof(buf)-n] -= (unsigned char)plen;
		buf[sizeof(buf)-n+2] = (buf[sizeof(buf)-1] & 1) ? 0x03 : 0x02;
	}
  #endif
	return wpabuf_alloc_copy(der, (size_t)len);
}
